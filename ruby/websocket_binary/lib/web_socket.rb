...
# Modifications by Jonathan Palley, 2012 to support elements of the IETF websocket final draft on client
...
class WebSocket
    ...
    def initialize(arg, params = {})
        ...
        write(
          ...
          "Sec-WebSocket-Protocol: sensocol1\r\n" +
          "Sec-WebSocket-Version: 13\r\n"
          
        )
        if params[:username]
          write(
            "Authorization: Basic #{Base64.encode64("#{params[:username]}:#{params[:password]}")}\r\n"
          )
        end
        flush()
        #write(
        ...
        #TODO: Verify Sec-WebSocket-Key is correct
        @handshaked = true
    ...
    def encode data, opcode=OPCODE_TEST
      frame = []
      frame << (opcode | 0x80)
    ...
    def send_binary(data)
      write(encode(data, OPCODE_BINARY))
      flush()
    end
    def send(data)
      if !@handshaked
