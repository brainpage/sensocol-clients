 ...
 *
 *
 * This file was originally a part of Apache Avro
 */
...
static void dump_line(FILE * out, const char *addr, const long len)
{
	...
	for (i = 0; i < 16; i++) {
		if (i < len) {
			fprintf(out, " %02X", ((unsigned char *) addr)[i]);
		} else {
			fprintf(out, " ..");
		}
		if (!((i + 1) % 8)) {
	...
	fprintf(out, "\t");
	for (i = 0; i < 16; i++) {
		char c = 0x7f & ((unsigned char *) addr)[i];
		if (i < len && isprint(c)) {
			fprintf(out, "%c", c);
		} else {
			fprintf(out, ".");
...
{
  dump(stdout,(char *)packet->buffer, packet->buffer_offset);
}
